...
{
    m_backgroundContainer->setWidth(sTerm->columns() * iFontWidth);
    m_backgroundContainer->setHeight(sTerm->rows() * iFontHeight);
    m_textContainer->setWidth(sTerm->columns() * iFontWidth);
    m_textContainer->setHeight(sTerm->rows() * iFontHeight);
    m_overlayContainer->setWidth(sTerm->columns() * iFontWidth);
    m_overlayContainer->setHeight(sTerm->rows() * iFontHeight);

    // If the height grows, make sure we have enough rows
    if (m_rows.size() < sTerm->rows()) {
        m_rows.resize(sTerm->rows());
        m_rowContents.resize(sTerm->rows());
    }
...
void TextRender::paintFromBuffer(QList<QList<TermChar> >& buffer, int from, int to, qreal &y)
{
    ...
    for(int i=from; i<to; i++) {
        ...
        // Ensure the row has enough BG & FG delegates for the worst possible
        // case (unique contents in every cell).
        for (; m_rows[i].size() < xcount; ) {
        ...
        for(int j=0; j<xcount; j++) {
            ...
            if (currAttrib.attrib != nextAttrib.attrib ||
                ...
                j==xcount-1)
            {
                QQuickItem *backgroundRectangle = m_rows[i][xDelegateIndex++];
        ...
        for (int j=xDelegateIndex;j<xcount; j++) {
            m_rows[i][j++]->setVisible(false);
        }
        ...
        for (int j=0; j<xcount; j++) {
            ...
            if (currAttrib.attrib != nextAttrib.attrib ||
                ...
                j==xcount-1)
            {
                QQuickItem *foregroundText = m_rowContents[i][xDelegateIndex++];
        ...
        for (int j=xDelegateIndex;j<xcount; j++) {
            m_rowContents[i][j]->setVisible(false);
        }
    }
}
